...
This data set contains set of traffic announcements entered into the system by the Transport Management Center operators from 2018 to 2020. There are 12 variables and 58422 observations.

[Project Proposal Link](https://pjournal.github.io/mef04g-a-k-a-r/Project-Proposal.html)

## Preprocessing

The first step was combining the excel files, since there were two separate data sets.
```{r,warning=FALSE,message=FALSE}
DS_1 <- read_excel("2018-2019.xlsx")
DS_2 <- read_excel("2020.xlsx")
...
The values of the *Announcement_Types* were converted to ENG with using *stringr*. 
```{r,warning=FALSE,message=FALSE}
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Kaza Bildirimi","Accident Notification")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Yoğun Trafik","Traffic Jam")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Araç Arızası","Vehicle Breakdown")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Haber","News")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Bakım-Onarım Çalışması","Maintenance")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Çevre Düzenlemesi","Landscaping")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Araç Yangını","Vehicle Fire")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Yolun Trafiğe Kapanması","Road Closure")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Şehir Hatları Sefer Bildirisi","Marine Traffic Information-State")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"İDO Sefer Bildirisi","Marine Traffic Information-Private")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Yol Yapım Çalışması","Construction Work-Road")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Alt Yapı Çalışması","Infrastructure Work")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Yolu Etkileyen Hava Koşulu","Weather Condition")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"İmalat Çalışması","Construction Work-Structure")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Diğer","Others")
DS$Announcement_Types=str_replace(DS$Announcement_Types,"Kontrollü Kapama (Protokol)","Road Closure-Protocol")
```

The data type of the *Announcement_Types* were converted from char to factor.
```{r,warning=FALSE,message=FALSE}
DS$Announcement_Types= factor(DS$Announcement_Types,levels= c("Accident Notification","Traffic Jam","Vehicle Breakdown","News","Maintenance",
...
```{r,warning=FALSE,message=FALSE}
DS$Accident_Types=str_replace(DS$Accident_Types,"trafik kazası (hasarlı)"," Vehicle Damage Only")
DS$Accident_Types=str_replace(DS$Accident_Types,"trafik kazası (yaralanmalı)","w/ Injuries")
DS$Accident_Types=str_replace(DS$Accident_Types,"trafik kazası (zincirleme)","Pile-up Accident")
DS$Accident_Types=str_replace(DS$Accident_Types,"trafik kazası (can kaybı)","w/ Fatality")
DS$Accident_Types=str_replace(DS$Accident_Types,"Seçiniz","Others")
```

The data type of the *Accident_Types* were converted from char to factor.
```{r,warning=FALSE,message=FALSE}
DS$Accident_Types <- factor(DS$Accident_Types,levels = c("Vehicle Damage Only", "w/ Injuries", 
                                                         "Pile-up Accident", "w/ Fatality",
                                                         "Others")) 
```
...
P1_1 <- P1 %>% filter(Problem_Index >= 0) %>% 
  ...
  Count_Announcement_Types= n(), Sum_Problem_Index=sum(Problem_Index))%>% filter(Avg_Problem_Index > 0) 
...
```{r,warning=FALSE,message=FALSE}
ggplot(P1_1,aes(x="", y=Avg_Problem_Index, fill=Announcement_Types)) +geom_bar(stat='identity',width = 1)+coord_polar("y")
```
...
```{r,warning=FALSE,message=FALSE}
ggplot(P2_1, aes(x=Year_Quarter, y=n)) + geom_bar(stat="identity", position="dodge",fill="#d3501d", colour="black")+ theme_minimal()
```
...
As a third section, it is investigated whether there is an effect of the pandemic on the number of Accidents and is there a seasonal trend on the number of Accidents.

*Since the November of 2020 is not completed yet, is not included in the study and date-related recording errors are kept on the subset. *

```{r,warning=FALSE,message=FALSE,echo=T, results='hide'}
P3=setDT(DS)[, Year_Month := format(as.Date(`Starting_Time`), "%Y-%m") ]
P3=setDT(P3)[, Year := format(as.Date(`Starting_Time`), "%Y") ]
P3=setDT(P3)[, Month := format(as.Date(`Starting_Time`), "%m") ]
P3=setDT(P3)[, Day := format(as.Date(`Starting_Time`), "%d") ]

P3_1 = P3 %>% filter(Announcement_Types == "Accident Notification" & Effected_Lanes >= 0 & Year_Month != "2020-11") %>% group_by(Year_Month,Year,Month) %>% summarise(n = n())
...
```{r,warning=FALSE,message=FALSE}
ggplot(P3_1,aes(x=Year_Month,y = n)) + geom_bar(stat="identity",aes(fill=Year))+theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
* Besides, there is a peak on 2018-07 that is seen on the chart. To investigate in more detail, the daily distribution of the Accidents in 2018-07 is plotted.
...
In the final section, official holiday events are introduced to the data set in order to define holiday periods in the data. For instance, 'Christmas', 'Labor Day', 'Ramadan', 'National Sovereignty and Children's Day' and 'May 19 Commemoration of Atatürk and Youth and Sports Day'. 

In the data set, weeks that are includes any of the holiday event are taken into consideration.

Announcements that are reported during the holiday week are counted. Then, total number of announcements are shown on the pie chart in order to interpret the most common announcement reasons in the holiday term.
...
```{r,warning=FALSE,message=FALSE}
P4 <- DS %>%
  mutate(weekStarting_Time = lubridate::week(DS$Starting_Time), yearDS = lubridate::year(DS$Starting_Time))
  
P4_1 <-  mutate(P4, holidayWeek = (P4$weekStarting_Time==holidayCalendar$WeekHolidayStartDate & P4$yearDS==holidayCalendar$Year | P4$weekStarting_Time==holidayCalendar$WeekHolidayEndDate & P4$yearDS==holidayCalendar$Year)) %>% 
  filter(Effected_Lanes >= 0 & holidayWeek==TRUE) %>%
  group_by(Announcement_Types) %>% count(Announcement_Types)
```

The number of 'Announcment Types' during holiday weeks are shown in the below pie chart. 

```{r,warning=FALSE,message=FALSE}  
ggplot(P4_1,aes(x="", y=n, fill=Announcement_Types)) +geom_bar(stat='identity',width = 1)+coord_polar("y")+scale_fill_hue(l=85)
```
...
[Data Source](https://data.ibb.gov.tr/dataset/ulasim-yonetim-merkezi-uym-tarafindan-sisteme-girilen-trafik-duyurulari)